...
if (!action.querySelector('option[value="delegate"]')) action.insertAdjacentHTML("beforeend", '<option value="delegate">Delegate verified implementation</option>');
if (!action.querySelector('option[value="pr-view"]')) action.insertAdjacentHTML("beforeend", '<option value="pr-view">View GitHub pull request</option>');

function updateActionLabel() {
  const labels = { analyze: "Analyze change", chat: "Chat with repository", ask: "Ask repository", "fleet-ask": "Run evidence fleet", "fleet-plan": "Run planning fleet", report: "Review report", security: "Repository security scan", "bundle-verify": "Verify review capsule", "plan-history": "Inspect plan history", walkthrough: "Generate walkthrough", erd: "Generate schema impact", resolve: "Resolve review threads", docstrings: "Generate docstrings", consensus: "Run consensus gate", review: "Review local changes", conflicts: "Resolve merge conflicts", agent: "Sandbox fix and verify", task: "Implement GitHub issue", implement: "Implement local request", delegate: "Delegate verified implementation", recipe: "Run finishing-touch recipe", autofix: "Review-thread autofix", plan: "Generate plan", "work-plan": "Plan free-form request", fix: "Suggest safe fix", simplify: "Simplify changed code", tests: "Generate tests", init: "Initialize repository", "auth-status": "Integration auth status", "sessions-list": "List local sessions", "sessions-compact": "Compact a local session", "sessions-checkpoints": "Inspect session checkpoints", benchmark: "Benchmark review history", search: "Search local timeline", plugins: "Discover plugins and extensions", skills: "Validate repository skills", mcp: "Inspect MCP configuration", hooks: "Inspect lifecycle hooks", lsp: "Inspect LSP configuration", tasks: "Start background task", "tasks-list": "List background tasks", "pr-view": "View GitHub pull request" };
  const criteriaLabel = document.querySelector('label[for="criteria"]');
  button.innerHTML = `${labels[action.value]} <span>&rarr;</span>`;
  const local = ["review", "agent", "conflicts", "ask", "fleet-ask", "fleet-plan", "report", "security", "plan-history", "implement", "work-plan", "init", "auth-status", "sessions-list", "sessions-compact", "sessions-checkpoints", "benchmark", "search", "plugins", "skills", "mcp", "hooks", "lsp", "complete", "stats", "prompts", "tasks-list"].includes(action.value);
  targetLabel.textContent = action.value === "ask" || action.value === "chat" || action.value === "fleet-ask" || action.value === "fleet-plan" ? "Repository question or request" : action.value === "bundle-verify" ? "Review bundle JSON" : action.value === "work-plan" || action.value === "delegate" ? "Natural-language change request" : action.value === "complete" ? "File path" : action.value === "pr-view" ? "GitHub pull request" : local ? "Repository path" : action.value === "plan" ? "Change request or issue" : action.value === "task" ? "GitHub issue" : "GitHub pull request";
  ...
  verify.disabled = !["agent", "task", "implement", "delegate", "recipe", "autofix"].includes(action.value);
  if (!["agent", "delegate"].includes(action.value)) verify.value = "";
...
action.addEventListener("change", () => {
  if (action.value === "security-review") {
    ...
    targetLabel.textContent = "Repository path";
    input.placeholder = "C:\\path\\to\\repository";
  } else if (action.value === "delegate") {
    button.innerHTML = "Delegate verified implementation <span>&rarr;</span>";
...
button.addEventListener("click", async () => {
    const target = action.value === "auth-status" ? "." : ["review", "agent", "conflicts", "security", "security-review", "findings", "doctor", "init", "sessions-list", "benchmark", "plugins", "skills", "mcp", "hooks", "tasks-list", "bundle-verify"].includes(action.value) ? (input.value.trim() || repo.value.trim()) : input.value.trim();
  if (!target) return;
  button.disabled = true;
  button.textContent = "Working...";
  result.classList.add("hidden");
  try {
    const output = await window.__TAURI__.core.invoke("run_cli", { commandName: action.value, prUrl: target, model: model.value || null, provider: provider.value || null, effort: effort.value || null, profile: profile.value || null, agent: agentProfile.value || null, directories: directories.value || null, relatedRepos: relatedRepos.value || null, repoPath: action.value === "research" || action.value === "search" || action.value === "complete" ? (repo.value || ".") : ["review", "agent", "conflicts", "report", "security", "security-review", "findings", "doctor", "init", "auth-status", "sessions-list", "sessions-compact", "sessions-checkpoints", "benchmark", "plugins", "skills", "mcp", "hooks", "lsp", "stats", "prompts", "tasks", "tasks-list", "fleet-ask", "fleet-plan", "delegate"].includes(action.value) ? (repo.value || target) : (repo.value || null), criteria: criteria.value || null, verify: verify.value || null, externalSecurity: externalSecurity.checked, codeqlDatabase: codeqlDatabase.value || null, mcp: mcp.checked, webSearch: webSearch.checked, apply: apply.checked, remember: remember.checked, reReview: reReview.checked, sessionId: action.value === "chat" ? chatSessionId : null });
    empty.classList.add("hidden");
    if (action.value === "pr-view") {
      result.innerHTML = `<h2>GITHUB PULL REQUEST</h2><p>${escapeHtml(output.state.toUpperCase())}${output.merged ? " &middot; merged" : ""}${output.draft ? " &middot; draft" : ""} &middot; ${escapeHtml(output.headBranch)} @ ${escapeHtml(output.headSha)}</p><div class="evidence"><div class="row"><span>${escapeHtml(output.title)}</span><code>${escapeHtml(output.baseBranch)}</code><span class="badge">${output.checks} CHECKS</span></div><div class="row"><span>${output.changedFiles} changed files</span><code>+${output.additions}/-${output.deletions}</code><span class="badge">${output.unresolvedReviewThreads} OPEN THREADS</span></div></div>`;
    ...
    } else if (action.value === "skills") {
      result.innerHTML = `<h2>REPOSITORY SKILLS</h2><p>${output.skills.length} checked-in skill(s) &middot; ${output.valid ? "valid" : "needs attention"}</p><div class="evidence">${(output.skills || []).map((skill) => `<div class="row"><span>${escapeHtml(skill.name)}<br /><small>${escapeHtml(skill.description || skill.issues.join("; "))}</small></span><code>${skill.bytes} bytes</code><span class="badge">${skill.valid ? "VALID" : "WARN"}</span></div>`).join("") || "No repository skills found."}</div>`;
    } else if (action.value === "mcp") {
      result.innerHTML = `<h2>MCP CONFIGURATION</h2><p>${output.servers.length} configured server(s) &middot; ${output.valid ? "valid" : "needs attention"}</p><div class="evidence">${(output.servers || []).map((server) => `<div class="row"><span>${escapeHtml(server.name)}<br /><small>${escapeHtml(server.url)}#${escapeHtml(server.tool)}</small></span><code>${server.argumentKeys.length} args / ${server.headerCount} headers</code><span class="badge">${output.valid ? "VALID" : "WARN"}</span></div>`).join("") || "No MCP servers configured."}</div>`;
    } else if (action.value === "hooks") {
      result.innerHTML = `<h2>LIFECYCLE HOOKS</h2><p>${output.enabled ? "enabled" : "disabled"}</p><div class="evidence"><div class="row"><span>Before review</span><code>${escapeHtml((output.beforeReview || []).join(", ") || "none")}</code><span class="badge">ALLOWLISTED</span></div><div class="row"><span>After review</span><code>${escapeHtml((output.afterReview || []).join(", ") || "none")}</code><span class="badge">ALLOWLISTED</span></div></div>`;
    } else if (action.value === "benchmark") {
      ...
      result.innerHTML = `<h2>DELEGATED SESSION ${escapeHtml(output.status.toUpperCase())}</h2><p>Verification: ${escapeHtml(output.verify)} &middot; ${output.apply ? "apply enabled" : "sandbox only"}</p><p>${escapeHtml(output.request)}</p><div class="evidence"><div class="row"><span>Session ID<br /><small>${escapeHtml(output.id)}</small></span><code>${escapeHtml(output.initialHeadSha || "pending")}</code><span class="badge">${output.result?.proof ? "PROOF VERIFIED" : "PROOF PENDING"}</span></div><div class="row"><span>Proof artifact</span><code>${escapeHtml(output.proofPath)}</code><span class="badge">DURABLE</span></div></div>`;
