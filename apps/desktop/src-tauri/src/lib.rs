...
fn cli_args(
    ...
    if command == "implement" {
        let repo = repo_path.filter(|value| !value.trim().is_empty()).ok_or_else(|| String::from("Implementation agent requires an explicit repository path."))?;
    ...
    if command == "work-plan" {
        let repo = repo_path.filter(|value| !value.trim().is_empty()).ok_or_else(|| String::from("Work planning requires an explicit repository path."))?;
        let mut args = vec![command.to_string(), pr_url, "--repo".to_string(), repo];
        if let Some(model) = model.filter(|value| !value.trim().is_empty()) { args.extend(["--model".to_string(), model]); }
        if let Some(provider) = provider.filter(|value| !value.trim().is_empty()) { args.extend(["--provider".to_string(), provider]); }
        if let Some(agent) = agent.filter(|value| !value.trim().is_empty()) { args.extend(["--agent".to_string(), agent]); }
    ...
    if command == "sessions-compact" || command == "sessions-checkpoints" {
        let repo = repo_path.filter(|value| !value.trim().is_empty()).ok_or_else(|| String::from("Session lifecycle actions require an explicit repository path."))?;
        if pr_url.trim().is_empty() { return Err(String::from("A session ID is required.")); }
        let subcommand = if command == "sessions-compact" { "compact" } else { "checkpoints" };
        return Ok(vec!["sessions".to_string(), subcommand.to_string(), pr_url, "--repo".to_string(), repo, "--json".to_string()]);
    }
