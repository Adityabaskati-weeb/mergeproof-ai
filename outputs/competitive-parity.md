...
| Session continuity | Append-only local JSONL chat sessions with `sessions list|show` and `chat --session`; prompts are bounded and provenance traces are retained | Session persistence, `/resume`, memory and compaction | Review plans/history and adaptive learnings |
...
MergeProof is not a literal replacement for proprietary hosted surfaces. It does not reproduce Copilot's inline autocomplete, hosted cloud-agent runtime, remote steering, or GitHub-managed enterprise policy plane; it also does not reproduce CodeRabbit's hosted knowledge administration, managed billing/tenant controls, or hosted dashboard service. Its local equivalents are explicit and inspectable: bounded evidence, resumable sessions, parallel fleet/quorum runs, verified ephemeral worktrees, local outcomes, and conservative mutation boundaries.
