...
37. LSP is treated as an evidence source with a process boundary: configuration is inspectable and executable availability is testable, but diagnostics cannot enter a decision unless supplied as a bounded, repository-local artifact.
...
46. The same bounded autopilot correction loop is available from CLI, desktop, and VS Code: isolated attempts can converge only through verification and evidence re-review, and checkout mutation remains an explicit final action.
47. Shadow mode is a rollout safety valve: teams can observe the exact evidence gate and attestation on live pull requests without allowing an automated review request to block a merge, then switch one policy field to enforcement.
48. PR lifecycle actions share the same guardrails: creating a PR requires a clean branch and optional verification, fixes run in an isolated worktree, and autopilot can only apply a converged verified patch.
49. CodeRabbit-style warning checks remain visible without masquerading as merge-blocking errors; the publication layer distinguishes non-blocking uncertainty from evidence failures.
50. Post-merge action output is durable and digest-addressed, while webhook execution is disabled unless the operator explicitly enables it; a model report cannot execute arbitrary repository commands.
