...
| Client surfaces | Interactive CLI chat with resumable local sessions, signed read-only remote turns, machine-readable `chat-turn` bridge, desktop evidence chat plus fleet ask/plan actions, one-shot CLI, VS Code, Cursor plugin metadata/rule, JetBrains plugin source, CI, and installable agent skill | GitHub, IDE, CLI, cloud agent, remote control, fleet | Git platforms, IDE, CLI, Slack |
...
| Session continuity | Append-only local JSONL chat sessions with `sessions list|show`, `chat --session`, and signed read-only `/session/turn` control | Session persistence, `/resume`, memory and compaction | Review plans/history and adaptive learnings |
