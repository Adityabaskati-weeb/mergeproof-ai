...
| Initialization / auth | Idempotent `init` scaffolds policy, safe mutation defaults, checks, and instructions; `auth status` reports provider/integration readiness without secrets and `auth login|logout --github` delegates to `gh` | `/init`, `/login`, `/logout`, `/env`, `/diagnose` | `cr auth status|login|logout` |
