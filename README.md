...
- `mergeproof override <github-pr-url> --check <name> --reason <text>` head-bound, auditable custom pre-merge check overrides
- `mergeproof pr view|create|fix|auto` Copilot-style GitHub PR lifecycle workflows with clean-checkout, verification, review, and publication gates
- `mergeproof post-merge <url>` executes configured read-only post-merge action reports, with optional provider comment publication and durable digests
...
- `mergeproof remote-delegate <start|status|cancel> ... --endpoint <url> --secret <secret>` for signed, verification-gated remote implementation handoff
...
- `mergeproof report [repository]` for local dashboard-style Markdown, JSON, self-contained offline HTML, or CSV review reports, natural-language custom reports, and optional Slack, Discord, Teams, or SendGrid email delivery
...
- `mergeproof override` records explicit custom-check exceptions against the exact PR head; the exception is visible in the analysis trace and cannot waive security findings or unresolved review threads
...
- Publish a GitHub review with an evidence-gated approval boundary: `APPROVE` is withheld unless the current head SHA, attestation, citations, criteria, resolved review threads, security gate, and custom checks all pass; explicit configured-check overrides are recorded and visible when used
...
The native desktop client lives in `apps/desktop`. Install Rust through `rustup` and the Tauri prerequisites before running `npm run desktop:dev` from the repository root. Use `npm run desktop:build` to create the Windows installers. The desktop action picker exposes evidence chat, analyze, read-only repository ask, consensus, local review, repository security scan, offline review-capsule verification, plan history, session compaction/checkpoints, background tasks, sandbox agent, GitHub issue tasks, named recipes, delegated implementation, URL-based plan, free-form work plan, safe-fix, simplify, and test workflows through the same bundled CLI.
...
The manual `.github/workflows/mergeproof-task.yml` workflow implements a GitHub issue from retrieved repository evidence in an ephemeral worktree. It requires an allowlisted verification command, uploads the machine-readable result, comments on the source issue, and only pushes a separate handoff PR when the explicit `create_pr` input is enabled. The `.github/workflows/mergeproof-delegate.yml` workflow accepts a natural-language implementation request, runs the proof-carrying delegation on hosted compute, uploads the proof/result artifacts, and only opens a separate PR after the verified convergence gate passes.
...
Repository policy lives in `.mergeproof/config.json`; use `mergeproof configuration` to inspect it or `mergeproof configuration --generate` to create the starter policy explicitly. Existing CodeRabbit repositories can run `mergeproof configuration --from-coderabbit --repo . --json` for a bounded preview, then `--apply-coderabbit` to write the converted policy. The importer maps review profile, path guidance, linked repositories, pre-merge check names, automatic-review gates, custom finishing-touch recipes, and post-merge actions into the local policy; it reports unsupported hosted settings and never overwrites an existing policy unless `--force` is explicit. A repository policy can inherit up to three levels of bounded JSON policy files through `"extends": "../organization-policy.json"`; `MERGEPROOF_CENTRAL_CONFIG` can provide an organization policy when the checkout does not contain one. Local scalar values override inherited values, while custom checks are combined and deduplicated before analysis. Team review guidance can be added to `.mergeproof/instructions.md`. Copy `.mergeproof/checks.example.json` to `.mergeproof/checks.json` to add bounded natural-language pre-merge checks; each check becomes a normal evidence criterion and cannot produce a ready decision without valid citations. Supported policy keys are `provider`, `model`, `effort`, `retrievalTopK`, `minCitationsPerCriterion`, `pathFilters`, `requestChangesWorkflow`, `highLevelSummary`, `reviewMode` (`enforce` or `shadow`), `autoReview`, `autoReviewDescriptionKeyword`, `autoIncrementalReview`, `autoPauseAfterReviewedCommits`, `ignoreTitleKeywords`, `reviewLabels`, `includeDrafts`, `baseBranches`, `ignoreUsernames`, optional inline `customChecks`, and optional `postMergeActions` with bounded read-only prompts. Shadow mode runs the complete evidence gate but publishes a neutral check and skips automated review requests, allowing a team to measure calibration before enforcement. Review effort defaults to `medium`; `low` uses four repository chunks, `medium` eight, and `high` sixteen unless `--retrieval-top-k` is explicitly set.
...
The review stream is agent-friendly: `mergeproof review . --agent-output` emits one JSON object per line for `review_context`, `status`, `finding`, and `complete`, including source citations and the attestation digest. Use `sessions fork`, `sessions export --format markdown|json`, `sessions compact <id> --keep 20`, `sessions checkpoints <id>`, `sessions delete`, and `sessions delete-all --yes` to manage local transcripts. Compaction archives older turns in `.mergeproof/sessions/<id>.archive.jsonl` and leaves a digest-bearing checkpoint in the active transcript; it never silently discards history. Knowledge can be proposed with `knowledge owner/repo --propose "..."`, inspected with `--proposals`, and explicitly approved or rejected; pending proposals are never retrieved into a review. Slack `learn` requests follow the same pending-approval path unless `MERGEPROOF_SLACK_LEARN_AUTO_APPROVE=true` is explicitly set. `mergeproof doctor --repo .` checks runtime, GitHub auth, model credentials, writable storage, optional web search, and Cargo availability without printing secret values. `mergeproof research` uses network only when Tavily or Brave credentials are explicitly configured; otherwise it reports that research is unavailable. Long-running read-only work can be detached with `mergeproof tasks start review --repo .`, `tasks list`, `tasks show <id>`, and `tasks cancel <id>`. Task records and bounded logs stay under `.mergeproof/tasks/`; the task allowlist rejects arbitrary shell commands and cancellation is preserved as an operator decision.
...
Mutation actions are explicit: `--publish-review` posts a GitHub review or fallback PR comment, `--publish-summary` updates only the `mergeproof-summary` marker block in a GitHub PR body, `--create-gitlab-issue` creates a GitLab follow-up using `GITLAB_TOKEN`, and `--create-jira` creates a Jira follow-up using `JIRA_PROJECT_KEY`. GitHub approval is proof-gated at publication time: stale-head, missing/invalid-attestation, unsupported-claim, blocking-failure, warning-check, unresolved/unavailable review threads, uncited-criterion, or medium/high security/quality conditions downgrade an otherwise ready result to a non-approving comment. These flags are never enabled by default.
...
The VS Code extension exposes `review`, `analyze`, URL-based `plan`, free-form `work-plan`, `fix`, `tests`, guarded `autofix`, bounded `autopilot`, and offline review-capsule verification commands from the command palette. Local review includes uncommitted files and uses the same validator as PR analysis, so desktop, terminal, CI, and editor results share one evidence contract. GitHub issue-comment commands include `/mergeproof review`, `/mergeproof full review`, `/mergeproof summary`, `/mergeproof diagram`, `/mergeproof plan`, `/mergeproof implement <request>`, `/mergeproof issue`, and `/mergeproof help`; only the explicit `implement` command creates a separate PR.
...
Custom pre-merge checks accept `mode: "off"`, `"warning"`, or `"error"`; warning-only failures remain visible but do not request changes or fail the provider status.
