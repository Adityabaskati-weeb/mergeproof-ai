...
MergeProof is intentionally local-first and evidence-gated. The repository includes a root `plugin.json` plus Claude, Cursor, GitHub agent, command, and skill packaging so the same review contract can travel across supported agent clients. It still does not claim to replace GitHub's inline autocomplete, hosted cloud-agent fleet, enterprise policy plane, or CodeRabbit's hosted knowledge administration, billing/tenant controls, and dashboard service. Those hosted surfaces require their own provider infrastructure; MergeProof's differentiator is an inspectable evidence ledger that makes every decision, external diagnostic, model output, and mutation boundary auditable.
