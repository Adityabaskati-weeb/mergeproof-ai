...
export type ModelCompletionContext = { filePath: string; language: string; before: string; after: string; request?: string; repository?: string; headSha?: string };
export type ModelCompletion = { completion: string };
export type ModelProvider = { name: string; analyze: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelAnalysis>; plan: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelPlan>; fix: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelFix>; task: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelFix>; implement: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelFix>; recipe: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelFix>; simplify: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelFix>; resolve: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelFix>; tests: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelFix>; docs: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelFix>; answer: (context: ModelQuestionContext, signal?: AbortSignal) => Promise<ModelAnswer>; complete?: (context: ModelCompletionContext, signal?: AbortSignal) => Promise<ModelCompletion> };
...
function implementationPrompt(context: PullRequestContext, criteria: string[]): string {
  return JSON.stringify({ task: "Implement the user's request in this clean local repository. First infer the smallest safe change from the supplied repository evidence and instructions, then return a minimal unified diff. You may modify existing files represented by evidence and add focused source or test files when required. Do not modify secrets, dependency lockfiles, CI credentials, generated artifacts, or unrelated files. Include or update focused tests when the evidence supports it. If the request is ambiguous or a safe implementation cannot be grounded in the supplied evidence, return an empty patch.", request: context.title, criteria, repositoryEvidence: context.repositoryEvidence ?? [], customInstructions: context.customInstructions ?? "", headSha: context.headSha });
}
...
function completionPrompt(context: ModelCompletionContext): string {
  return JSON.stringify({
    task: "Complete the code at the cursor. Return only the text to insert, with no markdown fences, explanation, or surrounding quotes. Preserve the file's style and do not invent imports or APIs that are not supported by the visible context.",
    ...
    afterCursor: context.after,
  });
}

async function completeWithOpenAI(model: string, context: ModelCompletionContext, compatible: boolean, signal?: AbortSignal): Promise<ModelCompletion> {
  const client = openAiClient(compatible);
  const system = "You are MergeProof Complete, a conservative code completion engine. Return only the smallest useful insertion at the cursor. Never include markdown fences, commentary, or a repeated prefix.";
  if (compatible) {
    const response = await client.chat.completions.create({ model, messages: [{ role: "system", content: system }, { role: "user", content: completionPrompt(context) }], temperature: 0.1, max_tokens: 1200 }, { signal });
    return { completion: response.choices[0]?.message.content?.trim() ?? "" };
  }
  const response = await client.responses.create({ model, input: [{ role: "system", content: system }, { role: "user", content: completionPrompt(context) }], temperature: 0.1, max_output_tokens: 1200 }, { signal });
  return { completion: response.output_text.trim() };
}

async function completeWithAnthropic(model: string, context: ModelCompletionContext, signal?: AbortSignal): Promise<ModelCompletion> {
  const response = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", signal, headers: { "content-type": "application/json", "x-api-key": process.env.ANTHROPIC_API_KEY ?? "", "anthropic-version": "2023-06-01" }, body: JSON.stringify({ model, max_tokens: 1200, temperature: 0.1, system: "You are MergeProof Complete, a conservative code completion engine. Return only the smallest useful insertion at the cursor. Never include markdown fences, commentary, or a repeated prefix.", messages: [{ role: "user", content: completionPrompt(context) }] }) });
  if (!response.ok) throw new Error(`Anthropic request failed with HTTP ${response.status}.`);
  const payload = await response.json() as { content?: Array<{ type?: string; text?: string }> };
  return { completion: payload.content?.find((item) => item.type === "text")?.text?.trim() ?? "" };
}
...
export function renderAnalysisPrompts(context: PullRequestContext, criteria: string[]): { system: string; user: string } {
  return { system: systemPrompt(), user: analysisPrompt(context, criteria) };
}
...
export function createModelProvider(model = process.env.OPENAI_MODEL || "gpt-5.6", provider = (process.env.MERGEPROOF_PROVIDER || "openai") as ModelProviderKind): ModelProvider {
  ...
  if (normalizedProvider === "anthropic") {
    ...
    return { name: `anthropic:${model}`, analyze: (context, criteria, signal) => analyzeWithAnthropic(model, context, criteria, signal), plan: (context, criteria, signal) => planWithAnthropic(model, context, criteria, signal), fix: (context, criteria, signal) => fixWithAnthropic(model, context, criteria, signal), task: (context, criteria, signal) => fixWithAnthropic(model, context, criteria, signal, taskPrompt), implement: (context, criteria, signal) => fixWithAnthropic(model, context, criteria, signal, implementationPrompt), recipe: (context, criteria, signal) => fixWithAnthropic(model, context, criteria, signal, recipePrompt), simplify: (context, criteria, signal) => fixWithAnthropic(model, context, criteria, signal, simplifyPrompt), resolve: (context, criteria, signal) => fixWithAnthropic(model, context, criteria, signal, conflictPrompt), tests: (context, criteria, signal) => fixWithAnthropic(model, context, criteria, signal, testPrompt), docs: (context, criteria, signal) => fixWithAnthropic(model, context, criteria, signal, docstringPrompt), answer: (context, signal) => answerWithAnthropic(model, context, signal), complete: (context, signal) => completeWithAnthropic(model, context, signal) };
  }
