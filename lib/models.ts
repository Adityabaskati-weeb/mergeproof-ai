...
export type ModelProvider = { name: string; analyze: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelAnalysis>; plan: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelPlan>; fix: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelFix>; simplify: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelFix>; tests: (context: PullRequestContext, criteria: string[], signal?: AbortSignal) => Promise<ModelFix> };
...
export function createModelProvider(model = process.env.OPENAI_MODEL || "gpt-5.6", provider = (process.env.MERGEPROOF_PROVIDER || "openai") as ModelProviderKind): ModelProvider {
  ...
  if (normalizedProvider === "anthropic") {
    ...
    return { name: `anthropic:${model}`, analyze: (context, criteria, signal) => analyzeWithAnthropic(model, context, criteria, signal), plan: (context, criteria, signal) => planWithAnthropic(model, context, criteria, signal), fix: (context, criteria, signal) => fixWithAnthropic(model, context, criteria, signal), simplify: (context, criteria, signal) => fixWithAnthropic(model, context, criteria, signal, simplifyPrompt), tests: (context, criteria, signal) => fixWithAnthropic(model, context, criteria, signal, testPrompt) };
  }
  ...
  return { name: `${normalizedProvider}:${model}`, analyze: (context, criteria, signal) => analyzeWithOpenAI(model, context, criteria, normalizedProvider === "openai-compatible", signal), plan: (context, criteria, signal) => planWithOpenAI(model, context, criteria, normalizedProvider === "openai-compatible", signal), fix: (context, criteria, signal) => fixWithOpenAI(model, context, criteria, normalizedProvider === "openai-compatible", signal), simplify: (context, criteria, signal) => fixWithOpenAI(model, context, criteria, normalizedProvider === "openai-compatible", signal, simplifyPrompt), tests: (context, criteria, signal) => fixWithOpenAI(model, context, criteria, normalizedProvider === "openai-compatible", signal, testPrompt) };
}
