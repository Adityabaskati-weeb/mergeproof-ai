...
export async function publishChangeRequestCheck(prUrl: string, analysis: Analysis): Promise<string | undefined> {
  const target = parseChangeRequestUrl(prUrl);
  if (target.provider === "github") return publishPullRequestCheck(prUrl, analysis);
  const context = await fetchChangeRequest(target);
  const passed = analysis.decision === "ready";
  if (target.provider === "gitlab") {
    const { base, project } = gitlabBase(target);
    const response = await fetch(`${base}/projects/${project}/statuses/${encodeURIComponent(context.headSha)}`, { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded", ...authHeaders(target.provider) }, body: new URLSearchParams({ state: passed ? "success" : "failed", name: "MergeProof evidence gate", description: `MergeProof: ${analysis.decision}`, target_url: target.ref.url }).toString() });
  ...
  if (target.provider === "bitbucket") {
    const response = await postJson(`https://api.bitbucket.org/2.0/repositories/${encodeURIComponent(target.ref.owner)}/${encodeURIComponent(target.ref.repo)}/commit/${encodeURIComponent(context.headSha)}/statuses/build`, authHeaders(target.provider), { key: "mergeproof", name: "MergeProof evidence gate", state: passed ? "SUCCESSFUL" : "FAILED", description: `MergeProof: ${analysis.decision}`, url: target.ref.url }, "Bitbucket");
    return typeof response.url === "string" ? response.url : target.ref.url;
  }
  const { base, repository } = azureBase(target);
  const response = await postJson(`${base}/repositories/${repository}/commits/${encodeURIComponent(context.headSha)}/statuses?api-version=7.1`, authHeaders(target.provider), { state: passed ? "succeeded" : "failed", description: `MergeProof: ${analysis.decision}`, targetUrl: target.ref.url, context: { name: "MergeProof evidence gate", genre: "MergeProof" } }, "Azure DevOps");
  return typeof response.url === "string" ? response.url : target.ref.url;
}
