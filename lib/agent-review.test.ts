...
import { renderAgentReviewEvents, toAgentReviewEvents } from "./agent-review";
...
describe("agent review stream", () => {
  it("emits context, evidence findings, security findings, and completion", () => {
    ...
    expect(events.map((event) => event.type)).toEqual(["status", "review_context", "finding", "finding", "status", "complete"]);
