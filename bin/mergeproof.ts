...
program.command("complete").description("Generate a non-mutating code completion at a file cursor").argument("<file>", "File path inside the repository").option("--repo <path>", "Repository path", process.cwd()).option("--line <number>", "1-based cursor line", "1").option("--column <number>", "1-based cursor column", "1").option("--language <language>", "Language override").option("--request <text>", "Optional completion instruction").option("--content <text>", "Unsaved file content, used by editor integrations").option("--model <model>", "Model name").option("--provider <provider>", "openai, openai-compatible, or anthropic").option("--json", "Print machine-readable JSON").action(async (file, options) => {
  try {
    const result = await completeFile(file, options.model, { repoPath: options.repo, line: Number(options.line), column: Number(options.column), language: options.language, request: options.request, content: options.content, provider: options.provider });
    if (options.json) console.log(JSON.stringify(result, null, 2));
    else process.stdout.write(result.completion);
  } catch (error) {
    console.error(`MergeProof completion error: ${error instanceof Error ? error.message : "Code completion failed."}`);
    process.exitCode = 1;
  }
});
...
program.command("stats").description("Aggregate local review, finding, and outcome statistics").option("--repo <path>", "Repository path", process.cwd()).option("--json", "Print machine-readable JSON").action(async (options) => {
  try {
    const stats = await readReviewStats(options.repo);
    if (options.json) console.log(JSON.stringify(stats, null, 2));
    else console.log(`Reviews: ${stats.reviews.total}\nDecisions: ${Object.entries(stats.reviews.decisions).map(([key, value]) => `${key}=${value}`).join(", ") || "none"}\nModels: ${Object.entries(stats.reviews.models).map(([key, value]) => `${key}=${value}`).join(", ") || "none"}\nAttested: ${stats.reviews.attested}${stats.reviews.averageElapsedMs !== undefined ? `\nAverage latency: ${stats.reviews.averageElapsedMs}ms` : ""}\nFindings: ${stats.findings.total} (${stats.findings.open} open, ${stats.findings.ignored} ignored)\nOutcomes: ${stats.outcomes.total}`);
  } catch (error) {
    console.error(`MergeProof stats error: ${error instanceof Error ? error.message : "Statistics failed."}`);
    process.exitCode = 1;
  }
});

program.command("prompts").description("Replay explicitly saved local model prompts").option("--repo <path>", "Repository path", process.cwd()).option("--limit <number>", "Maximum prompt records", "20").option("--json", "Print machine-readable JSON").action(async (options) => {
  try {
    const prompts = await readPrompts(options.repo, Number(options.limit));
    if (options.json) console.log(JSON.stringify(prompts, null, 2));
    else console.log(prompts.map(renderPromptRecord).join("\n\n") || "No prompts saved. Use review or analyze with --save-prompts; prompt persistence is opt-in.");
  } catch (error) {
    console.error(`MergeProof prompts error: ${error instanceof Error ? error.message : "Prompt replay failed."}`);
    process.exitCode = 1;
  }
});
